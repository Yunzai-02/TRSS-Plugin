...
export class miHoYoLogin extends plugin {
	...
	miHoYoLoginDetect() {
		if (!config.miHoYoLogin.open) return false
	...
	async miHoYoLoginQRCode() {
		...
		try {
			...
			try {
				const avatarUrl = Bot.pickMember(this.e.group_id, String(this.e.user_id)).getAvatarUrl()
				...
				const avatarImage = sharp(avatarBuffer)

				const qrCodeMetadata = await qrCodeImage.metadata()
				const qrCodeWidth = qrCodeMetadata.width || 0
				const qrCodeHeight = qrCodeMetadata.height || 0

				const avatarSize = Math.floor(qrCodeWidth * 0.2)
				const x = Math.floor((qrCodeWidth - avatarSize) / 2)
				const y = Math.floor((qrCodeHeight - avatarSize) / 2)

				const circularAvatar = await avatarImage
				...
				finalImageBuffer = await qrCodeImage
					.composite([
						{
				...
				logger.error(this.e.logFnc, '获取头像失败，跳过头像合成')
			}

			const img = segment.image(`base64://${finalImageBuffer.toString('base64')}`)

			Running[this.e.user_id] = img
			this.reply(['请使用米游社扫码登录', img], true, {
				at: true,
				recallMsg: 60
			})
		} catch (err) {
		...
		for (let n = 1; n < 60; n++) {
			...
			try {
				...
				if (res.retcode != 0) {
					Running[this.e.user_id] = false
					return this.reply(
						['二维码已过期，请重新登录', segment.button([{ text: '米哈游登录', callback: '米哈游登录' }])],
						true,
	...
	miHoYoLoginHelp() {
		...
		this.reply(['发送【米哈游登录】', segment.button([{ text: '米哈游登录', callback: '米哈游登录' }])], true, {
			at: true
		})
	}
}
